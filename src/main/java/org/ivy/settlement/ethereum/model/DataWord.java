...
import static org.ivy.settlement.infrastructure.bytes.ByteUtil.*;
...
public final class DataWord implements Comparable<DataWord> {
    ...
    public static DataWord of(byte[] data) {
        ...
        var leadingZeroBits = numberOfLeadingZeros(data);
        var valueBits = 8 * data.length - leadingZeroBits;
        if (valueBits <= 8) {
    ...
    public int intValueSafe() {
        var bytesOccupied = bytesOccupied();
    ...
    public DataWord add(DataWord word) {
        var newData = new byte[32];
        for (int i = 31, overflow = 0; i >= 0; i--) {
            var v = (this.data[i] & 0xff) + (word.data[i] & 0xff) + overflow;
            newData[i] = (byte) v;
            overflow = v >>> 8;
        }
        return new DataWord(newData);
    ...
    public DataWord add2(DataWord word) {
        var result = value().add(word.value());
    ...
    public DataWord mul(DataWord word) {
        var result = value().multiply(word.value());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }
    ...
    public DataWord div(DataWord word) {
        ...
        var result = value().divide(word.value());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }
    ...
    public DataWord sDiv(DataWord word) {
        ...
        var result = sValue().divide(word.sValue());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }

    // TODO: improve with no BigInteger
    public DataWord sub(DataWord word) {
        var result = value().subtract(word.value());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }

    // TODO: improve with no BigInteger
    public DataWord exp(DataWord word) {
        var newData = value().modPow(word.value(), _2_256);
    ...
    public DataWord mod(DataWord word) {
        ...
        var result = value().mod(word.value());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }
    ...
    public DataWord addmod(DataWord word1, DataWord word2) {
        ...
        var result = value().add(word1.value()).mod(word2.value());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }
    ...
    public DataWord shiftLeft(DataWord arg) {
        ...
        var result = value().shiftLeft(arg.intValueSafe());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }
    ...
    public DataWord shiftRight(DataWord arg) {
        ...
        var result = value().shiftRight(arg.intValueSafe());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }
    ...
    public DataWord shiftRightSigned(DataWord arg) {
        ...
        var result = sValue().shiftRight(arg.intValueSafe());
        return new DataWord(ByteUtil.copyToArray(result.and(MAX_VALUE)));
    }
    ...
    public String shortHex() {
        var hexValue = Hex.toHexString(getNoLeadZeroesData()).toUpperCase();
    ...
    public int compareTo(DataWord o) {
        if (o == null) return -1;
        var result = FastByteComparisons.compareTo(
                data, 0, data.length,
    ...
    public DataWord signExtend(byte k) {
        ...
        var mask = this.sValue().testBit((k * 8) + 7) ? (byte) 0xff : 0;
        var newData = this.copyData();
        for (var i = 31; i > k; i--) {
            newData[31 - i] = mask;
        }
    ...
    public int bytesOccupied() {
        var firstNonZero = ByteUtil.firstNonZeroByte(data);
