...
public interface BorshInput {
    default public <T> T read(final Class klass) {
    ...
    default public <T> T readPOJO(final Class klass) {
        try {
    ...
    default public byte[] readFixedArray(final int length) {
        if (length < 0) {
            throw new IllegalArgumentException();
        }
        final byte[] bytes = new byte[length];
    ...
    default public <T> T[] readArray(final Class klass) {
        final int length = this.readU32();
    ...
    default public <T> Optional<T> readOptional(final Class klass) {
        final boolean isPresent = (this.readU8() != 0);
    ...
    default public void read(final byte[] result) {
